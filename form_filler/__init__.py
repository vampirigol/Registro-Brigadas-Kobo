...
class FormFiller:
    ...
    def __init__(self, mapping: dict[str, str] | None = None, page: Page | None = None):
        ...
        self._form_ctx = None   # contexto persistente para llenado de formularios
        self._form_page = None  # página persistente para llenado de formularios
    ...
    def stop(self, close_browser: bool = True) -> None:
        ...
        for closeable in (self._form_page, self._form_ctx):
            try:
                if closeable:
                    closeable.close()
            except Exception:
    ...
    def _ensure_page_alive(self, headless: bool | None = None) -> bool:
        """Verifica que la página siga viva; si fue cerrada, recrea el navegador."""
        if not self._page:
        ...
        try:
            ...
            logger.warning("La ventana del navegador fue cerrada. Reabriendo…")
    ...
    def fill_record(
        ...
        *,
        ...
        if not self._ensure_page_alive():
            logger.error("No se pudo abrir el navegador para la fila %d", row_index + 1)
            return False

        load_url = FORM_URL if USE_DIRECT_FORM_URL else APP_URL
        logger.info("Cargando formulario: %s (fila %d)", load_url, row_index + 1)

        browser = self._browser or self._page.context.browser
        # Nuevo contexto por fila: evita estados residuales del envío anterior.
        # Cerrar cualquier contexto/página previo para liberar recursos.
        for closeable in (self._form_page, self._form_ctx):
            try:
                if closeable:
                    closeable.close()
        ...
        form_ctx = browser.new_context(
            viewport={"width": 1400, "height": 900},
            ignore_https_errors=True,
        )
        form_page = form_ctx.new_page()
        form_page.set_default_timeout(45000)
        self._form_ctx = form_ctx
        self._form_page = form_page
        logger.info("Contexto de navegador creado para la fila (uno por fila)")
        ...
        try:
            ...
            try:
                form_page.wait_for_selector("form.or:not(.loading)", timeout=10000)
            ...
            if not candidates:
                ...
                candidates = _get_form_contexts_candidates(form_page)
            if not candidates:
                fallback = _get_form_frame(form_page)
                if fallback:
            ...
            for candidate_ctx, desc in candidates:
                ...
                if not _fill_consent_robust(candidate_ctx, form_page):
                ...
                fecha_ok = _fill_fecha_robust(candidate_ctx, fecha_value, form_page)
                if not fecha_ok:
            ...
            return self._wait_for_success(ctx, form_page)

        finally:
            pass  # el contexto se mantiene vivo para reutilizarlo en la siguiente fila
    ...
    def _wait_for_success(self, ctx: FormContext, page: Page, timeout: int = 15000) -> bool:
        """Espera confirmación de envío verificando indicadores reales del DOM.

        En lugar de un wait fijo de 1.5 s, espera hasta 4 s a que aparezca
        cualquier indicador de éxito o error y procede en cuanto lo detecta.
        """
        try:
            try:
                page.wait_for_selector(
                    ".vex-content, .alert-success, [id*='thank'], .form-complete, "
                    ".alert-danger, .or-constraint-msg.active",
                    timeout=4000,
                )
            except Exception:
                pass  # no apareció ningún indicador; continuamos con la comprobación manual
            ...
            success_by_url = "thank" in page_url.lower()
            success_by_dom = False
            try:
                success_by_dom = page.locator(
                    ".vex-content, .alert-success, [id*='thank'], .form-complete"
                ).count() > 0
            except Exception:
                pass
            ...
            if has_error:
                logger.warning("Errores de validación detectados tras envío")
                return False

            if success_by_url or success_by_dom:
                logger.info("Envío confirmado exitosamente")
                return True

            logger.info("Sin indicadores claros post-envío; asumiendo éxito")
            return True
        except Exception as e:
            logger.warning("Error esperando confirmación: %s", e)
            return True
